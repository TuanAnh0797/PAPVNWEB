...
<%--    <div class="content-header" style="padding: 5px;">
    ...
    <script>
        ...
        function DataHistoryTable(dataoftable) {
            var dataTable = $('#table_history').DataTable({
                ...
                "scrollY": "960px", // Chiều cao thanh cuộn
                "scrollCollapse": true,
                "paging": false, // Tắt phân trang
                "columnDefs": [
                    {
                        "targets": 1, // Áp dụng cho cột "Name"
                        "render": function (data, type, row, meta) {
                            // Tạo một thẻ <a> với dữ liệu từ cột "Name" làm nội dung
                            return '<a href="#" data-id="' + row.CodeBack + '">' + data + '</a>';
                        }
                    }
                ],
                "createdRow": function (row, data, dataIndex) {
                    // Kiểm tra giá trị của cột "Name" trong dòng
                    var JudgeValue = data.Judge;

                    // Nếu giá trị là "OK", đặt màu sắc của dòng thành xanh
                    if (JudgeValue === "OK") {
                        $('td', row).css('color', 'green').css('font-size', '20px');
                    }
                    else {
                        $('td', row).css('color', 'red').css('font-size', '20px');
                    }
                }
            });
                    // Xóa dữ liệu hiện tại trong bảng
                    dataTable.clear();
                    //var mdt = JSON.parse(data.d);
                    //dataTable.rows.add(data.d);
                    dataTable.rows.add(dataoftable);
                    // Vẽ lại bảng
                    dataTable.draw();
        };
