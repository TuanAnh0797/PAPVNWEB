                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        else if (typeplan == "2_12_6")
                        {
                            ...
                            for (DateTime currentHour = DateTime.Parse(TimeFrom.Trim() + ":00").AddHours(1); currentHour < DateTime.Parse(TimeTo.Trim() + ":00"); currentHour = currentHour.AddHours(1))
                            {
                                secwork = secwork - Config.TimeRest2Ca12_6h[currentHour.Hour] * 60;
                            }
                            if (DateTime.Parse(TimeTo.Trim() + ":00").Minute >= Config.TimeRest2Ca12_6h[DateTime.Parse(TimeTo.Trim() + ":00").Hour])
                            {
                                secwork = secwork - Config.TimeRest2Ca12_6h[DateTime.Parse(TimeTo.Trim() + ":00").Hour] * 60;
                            }
                            else
                            {
                                secwork = secwork - DateTime.Parse(TimeTo.Trim() + ":00").Minute * 60;
                            }
                            if (DateTime.Parse(TimeFrom.Trim() + ":00").Minute < Config.TimeRest2Ca12_6h[DateTime.Parse(TimeFrom.Trim() + ":00").Hour])
                            {
                                secwork = secwork - (Config.TimeRest2Ca12_6h[DateTime.Parse(TimeFrom.Trim() + ":00").Hour] - DateTime.Parse(TimeFrom.Trim() + ":00").Minute) * 60;
                            }
                            dBConnect.exnonquery("TA_sp_UpdateDateTimePlan", CommandType.StoredProcedure, ModelName.Trim(), TimeFrom.Trim() + ":00", TimeTo.Trim() + ":00", secwork, type);
                            return "1";
